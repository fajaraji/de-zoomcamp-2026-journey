...
You can use the provided Python script `load_yellow_taxi_data.py` or a Jupyter notebook to load the data into your GCS bucket.

* You will need to generate a Service Account with GCS Admin privileges or be authenticated with the Google SDK.
* Update the bucket name in the script.
* **Note:** If you are using orchestration tools such as Kestra, Mage, Airflow, or Prefect, **do not** load the data into BigQuery using the orchestrator.
* Make sure that all **6 files** (Jan-Jun 2024) show in your GCS bucket before beginning.

## BigQuery Setup

Before answering the questions, run the following SQL commands:
...
Write a query to count the distinct number of `PULocationID` for the entire dataset on both the tables.
...
0 Bytes. BigQuery saved Metadata so it does not need to read the table again to calculate the amount of rows.