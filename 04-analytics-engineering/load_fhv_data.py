...
BUCKET_NAME = "fajar-de-zoomcamp-hw4-2026"
    ...
    for service in SERVICES:
        for year in YEARS:
            for month in MONTHS:
                ...
                file_list.append({"url": url, "file_name": file_name})
    return file_list
...
if __name__ == "__main__":
    ...
    with ThreadPoolExecutor(max_workers=2) as executor:
        file_paths = list(executor.map(download_file, files_to_process))

    with ThreadPoolExecutor(max_workers=4) as executor:
        executor.map(upload_to_gcs, filter(None, file_paths))  # Remove None values

    print(f"All {len(files_to_process)} files processed and verified.")