...
columns:
  - name: pickup_datetime
    type: string
    description: "Trip start time (YYYY-MM-DD HH:MM:SS)"
    primary_key: true
    nullable: false
    checks:
      - name: not_null
  - name: dropoff_datetime
    type: string
    description: "Trip end time (YYYY-MM-DD HH:MM:SS)"
    primary_key: true
    nullable: false
  - name: taxi_type
    type: string
  ...
  - name: payment_type_id
    type: integer
    description: "Payment lookup key"
    primary_key: true
  - name: trip_distance
    type: float
...
custom_checks:
  - name: no_duplicate_trips
    description: "Ensure the staging table contains no duplicate composite keys"
    query: |
      SELECT COUNT(*)
      FROM (
        SELECT pickup_datetime, dropoff_datetime, taxi_type, payment_type_id, COUNT(*) AS cnt
        FROM staging.trips
        GROUP BY 1,2,3,4
        HAVING COUNT(*) > 1
      )
...
WHERE
    ...
    -- Membatasi jendela waktu sesuai parameter Bruin
    AND CAST(pickup_datetime AS TIMESTAMP) >= CAST('{{ start_datetime }}' AS TIMESTAMP)
    AND CAST(pickup_datetime AS TIMESTAMP) < CAST('{{ end_datetime }}' AS TIMESTAMP)
-- Jurus buang duplikat yang sudah disesuaikan dengan kemauan Satpam AI
QUALIFY ROW_NUMBER() OVER (
    PARTITION BY pickup_datetime, dropoff_datetime, taxi_type, payment_type_id
